    ...
    def _reward(self):
        ...
        self.object_state = np.array(
            p.getBasePositionAndOrientation(self.object_id)[0]).astype(
                np.float32)
        # self.object_state = [0.4, 0.3, 0.0]
        logging.info("self.object_state={}\n".format(self.object_state))
        ...
        #print(self.distance)
        distance_rate = self.pre_distance - self.distance
        reward_temp = distance_rate
        ...
        #如果机械臂一直无所事事，在最大步数还不能接触到物体，也需要给一定的惩罚
            ...
            self.reward = self.reward + reward_temp
            reward = self.reward
            self.terminated = False
